...
def create_app(config: dict = None) -> Flask:
    ...
    # Register all routes from the module-level app onto the new instance
    for rule in app.url_map.iter_rules():
        view_func = app.view_functions[rule.endpoint]
        flask_app.add_url_rule(rule.rule, rule.endpoint, view_func, methods=rule.methods)
    for code, handler in app.error_handler_spec[None][None].items() if app.error_handler_spec.get(None, {}).get(None) else []:
        flask_app.register_error_handler(code, handler)
    return flask_app
